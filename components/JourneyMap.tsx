...
export default function JourneyMap() {
    ...
    const compareByPeriod = (a: { year: string }, b: { year: string }) => {
    ...
    const experienceItems = resumeData.experience
        .map((e) => ({ ...e, title: e.role, subtitle: e.company, year: e.period }))
        .sort(compareByPeriod);

    const educationItems = resumeData.education
        .map((e) => ({ ...e, title: e.degree, subtitle: e.institution, year: e.year }))
        .sort(compareByPeriod);
    ...
    return (
        <section className="min-h-screen py-20 px-6 relative flex flex-col items-center">
            ...
            <motion.h2
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                className="text-4xl md:text-5xl font-cinematic text-[#f0e6d2] mt-24 mb-16 text-center drop-shadow-md"
            >
                Education
            </motion.h2>
